...
def make_step(net, step_size=1.5, end='conv5', channel=0,
    ...
    src.data[0] = np.roll(np.roll(src.data[0], ox, -1),
                          oy, -2)  # apply jitter shift
...
def iter_dream(net, img, blob, n_iter=50, channel=0):
    ...
    for i in xrange(100):
        ...
        PIL.Image.fromarray(np.uint8(frame)).save(
            "frames/{0}_{1:03d}_{2:04d}.jpg".format(blob, channel, frame_i))
...
class PreferedImage(object):
    ...
    def compute_pf_image(self, blob, channel, n_iter=30, octave_scale=1.4):
        print("creating prefered image for {} channel {:04d}".format(blob, channel))
        frame = self.init_image
        frame_i = 0
        # do not change scale when blob is fc
        if self.is_fc(blob):
            octave_scale=1
        h, w = frame.shape[:2]
        s = 0.05  # scale coefficient
        for i in xrange(n_iter):
            print("iter_{0:03d}".format(i))
            frame = deepdream(self.net, frame, end=blob, channel=channel, octave_scale=octave_scale)
            PIL.Image.fromarray(np.uint8(frame)).save(
                self.save_img(blob, channel, frame_i))
            return_frame = frame
            frame = nd.affine_transform(
                frame, [1 - s, 1 - s, 1], [h * s / 2, w * s / 2, 0], order=1)
            frame_i += 1
...
if __name__ == "__main__":
    ...
    model_path = '/home/nakamura/network_dissection/NetDissect/zoo/'
    prototxt = os.path.join(model_path, 'alexnet_imagenet_full_conv.prototxt')
    caffemodel = os.path.join(
        model_path, 'alexnet_imagenet_full_conv.caffemodel')
